...
const THEME_META = {
  light: { icon: "☀", short: "Light" },
  dark: { icon: "☾", short: "Dark" },
  system: { icon: "◐", short: "System" },
};
...
function updateThemePickerTrigger(preference) {
  ...
  if (!trigger) return;
  const meta = THEME_META[preference] || THEME_META.light;
  const code = trigger.querySelector(".theme-trigger-code");
  const name = trigger.querySelector(".theme-trigger-name");
  if (code) code.textContent = meta.icon;
  if (name) name.textContent = getThemeLabel(preference);
}

function ensureThemePickerHost() {
  if (document.getElementById("theme-picker")) return document.getElementById("theme-picker");

  const legacy = document.querySelector(".theme-switcher");
  if (legacy) {
    const host = document.createElement("div");
    host.id = "theme-picker";
    host.className = "theme-picker-host";
    legacy.replaceWith(host);
    return host;
  }

  const langHost = document.getElementById("lang-picker");
  if (!langHost?.parentElement) return null;

  const host = document.createElement("div");
  host.id = "theme-picker";
  host.className = "theme-picker-host";
  langHost.insertAdjacentElement("afterend", host);
  return host;
}
...
function buildThemePicker() {
  ...
  host.innerHTML = `
    <div class="theme-picker">
      ...
      <button type="button" class="theme-trigger glass-inset" id="theme-trigger" aria-haspopup="listbox" aria-expanded="false">
        <span class="theme-trigger-code" aria-hidden="true">☀</span>
        <span class="theme-trigger-name" data-i18n="theme.light">Light</span>
  ...
  THEME_OPTIONS.forEach((value) => {
    ...
    btn.innerHTML = `
      <span class="theme-option-icon" aria-hidden="true">${THEME_META[value].icon}</span>
